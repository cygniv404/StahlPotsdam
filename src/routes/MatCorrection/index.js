...
export default function ({ documentType }) {
  ...
  const loadDocument = (e) => {
    if (e.key === 'ArrowDown') {
      get(
        `${documentType}/${e.target.value === '' ? 0 : e.target.value}/prev`,
        (data) => {
          if (data.id) {
    ...
    if (e.key === 'ArrowUp') {
      get(
        `${documentType}/${e.target.value === '' ? 0 : e.target.value}/next`,
        (data) => {
          if (data.id) {
            setDocumentId(data.id);
            setClientData({
  ...
  const saveMatCorrection = (e) => {
    if (e.key === 'Enter') {
      post(
        { matCorrection },
        `${documentType}/mat_correction/${documentId}`,
        (data) => {
          if (data.id) {
  ...
  useEffect(() => {
    if (documentId !== '') {
      get(`${documentType}/${documentId}/position`, (data) => {
        if (data.length) {
      ...
      {positionsData.length > 0 && (
        <Grid xs={12}>
          <Box display="flex" justifyContent="start">
            <Button
              id={`input-${1 + positionsData.length + 1}`}
              variant="contained"
              color="primary"
