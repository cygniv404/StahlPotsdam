...
export default function ({ documentType }) {
  ...
  const handleInputChange = (e) => {
    if (Object.keys(clientData).indexOf(e.target.name) > -1) {
      setClientData({ ...clientData, [e.target.name]: e.target.value });
    } else if (Object.keys(costData).indexOf(e.target.name) > -1) {
      setCostData({
        ...costData,
        [e.target.name]: (e.target.name.indexOf('address') > -1 || e.target.name.indexOf('bv') > -1) ? e.target.value : parseInt(e.target.value, 10),
      });
    } else if (Object.keys(documentData).indexOf(e.target.name) > -1) {
  ...
  const handleDocumentIdChange = (e) => {
    ...
    get(
      `${documentType}/${e.target.value === '' ? 0 : parseInt(e.target.value, 10)}`,
      (data) => {
        if (data.id) {
  ...
  const handleCostIdChange = (e) => {
    ...
    get(
      `cost_center/${e.target.value === '' ? '0' : e.target.value.toString().replace('/', '_')}`,
      (data) => {
        if (data.id) {
          setCostData({
            ...costData,
  ...
  const loadDocument = (event) => {
    if (event.key === 'ArrowDown') {
      get(
        `${documentType}/${documentId === '' ? 0 : parseInt(documentId, 10)}/prev`,
        (data) => {
          if (data.id) {
    ...
    if (event.key === 'ArrowUp') {
      get(
        `${documentType}/${documentId === '' ? 0 : parseInt(documentId, 10)}/next`,
        (data) => {
          if (data.id) {
            setDocumentId(data.id);
            setClientId(data.client_id);
  ...
  const loadCost = (event) => {
    if (event.key === 'ArrowDown') {
      get(
        `cost_center/${parseInt(clientId, 10)}/${costId === '' ? '0' : costId.toString().replace('/', 'e')}/prev`,
        (data) => {
          if (data.id) {
            setCostId(data.id);
            setCostData({
    ...
    if (event.key === 'ArrowUp') {
      get(
        `cost_center/${parseInt(clientId, 10)}/${costId === '' ? '0' : costId.toString().replace('/', 'e')}/next`,
        (data) => {
          if (data.id) {
            setCostId(data.id);
            setCostData({
  ...
  const loadCostFromClient = (event) => {
    if (documentType === 'order' && event.key === 'Enter' && event.target.value !== '') {
      get(
        `client/${clientId === '' ? 0 : parseInt(clientId, 10)}/cost`,
        (data) => {
          if (data.client) {
            setClientData({
  ...
  const putDocument = (e) => {
    if (e.key === 'Enter') {
      post(
        {
          clientId: clientId === '' ? 0 : parseInt(clientId, 10),
        ...
        `${documentType}/${documentId === '' ? 0 : parseInt(documentId, 10)}`,
        (data) => {
          setDocumentId(data.id);
          setOpen(true);
        },
  ...
  const copyDocument = (e) => {
    if (e.key === 'F8') {
      post(
        ...
        `${documentType}/${documentId === '' ? 0 : parseInt(documentId, 10)}?copy=true`,
        (data) => {
          setDocumentId(data.id);
    ...
    <>
      ...
      <Grid item xs={2}>
        ...
        >
          ...
          >
            {`Neu ${documentType === 'order' || copiedOffer ? 'Auftrag' : 'Angebot'}nummer`}
          </DialogTitle>
          <DialogContent>
