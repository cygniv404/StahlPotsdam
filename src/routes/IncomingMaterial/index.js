...
export default function (props) {
    ...
    const handleArticleChange = (e) => {
        ...
        if (e.target.value.length > 0) {
            get(`article/${e.target.value.replace('_', '/')}`,
                (data) => {
                    if (data.id) {
    ...
    const loadArticle = (e) => {
        if (e.key === "ArrowDown") {
            get(`article/${articleId === '' ? 0 : articleId.replace('/', '_')}/prev`,
                (data) => {
                    if (data.id) {
                        setArticleId(data.id);
                        setArticleData(data)
        ...
        if (e.key === "ArrowUp") {
            get(`article/${articleId === '' ? 0 : articleId.replace('/', '_')}/next`,
                (data) => {
                    if (data.id) {
                        setArticleId(data.id);
                        setArticleData(data)
    ...
    const loadSupplier = (e) => {
        if (e.key === "ArrowDown") {
            get(`supplier/${supplierId === '' ? 0 : supplierId}/prev`,
                (data) => {
                    if (data.id) {
        ...
        if (e.key === "ArrowUp") {
            get(`supplier/${supplierId === '' ? 0 : supplierId}/next`,
                (data) => {
                    if (data.id) {
                        setSupplierId(data.id);
                        setSupplierDescription(data.name1)
                            ...
                            <TextField
                                ...
                                InputProps={{
                                    endAdornment: <InputAdornment
                                        position="start">€/{articleData.unit1 ?? 'to'}</InputAdornment>,
                                }}
                                disabled
                                variant="outlined"
                                className={classes.input2}
