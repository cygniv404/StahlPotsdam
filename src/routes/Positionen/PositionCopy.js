...
export default function (props) {
    ...
    const handleChangeCheckAll = (e) => {
        ...
        Object.keys(newPositionsCheck).map((item, index) => {
    ...
    const copyPosition = (e) => {
        if (e.key === 'Enter') {
            if (orderTargetId === '' || orderTargetId === orderSourceId || positionDescription.length === 0) {
                ...
                }, (error) => enqueueSnackbar('Fehler: Position(en) kann nicht kopiert werden!', {variant: "error"}))
            }
        }
    }
            ...
            <Grid item xs={2}>
                {
                    Object.keys(positionCheck).length > 0 && (
                        <FormControl component="fieldset">
                            <FormLabel
                                component="legend">{positionsData.length === 0 ? null : positionsData.length} Positionen</FormLabel>
                            <FormGroup>
                                <FormControlLabel
                                    control={<Switch id="input-3" checked={positionCheckAll}
                                                     onChange={handleChangeCheckAll}
                                                     name='check All' onKeyDown={switchAllPosition}/>}
                                    label="Alle Positionen"
                                />
                                {Object.keys(positionCheck).map((item, index) => {
                                    return (
                                        <FormControlLabel
                                            control={<Switch id={`input-${3 + index + 1}`} checked={positionCheck[item]}
            ...
            {positionsData.length > 0 && (
                <Grid xs={12}>
                    <Box display="flex" justifyContent="start">
                        <Button id={`input-${3 + positionsData.length + 1}`} variant="contained" color="primary"
