...
export default function ({bendTypeData, chosenEdge, dimX, dimY, showLabel, scale, defaultColor='#000',lineWidth = 1, ...rest}) {
    ...
    const draw = (ctx, dimX, dimY, scale, showLabel, lineWidth) => {
        if (ctx) {
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            if (showLabel && (bendTypeData['e' + i] > 0 || bendTypeData['f' + i] > 0)) {
                ...
                ctx.fillText(String.fromCharCode(96 + i),
    ...
    useEffect(() => {
        ...
        draw(context, dimX, dimY, scale, showLabel, lineWidth)
    }, [draw])
