...
export default function (props) {
    ...
    useEffect(() => {
        if (supplierData.payment_target) {
            setPaymentTarget(new Date(new Date(incomingDate).setTime(new Date(incomingDate).getTime() + parseInt(supplierData.payment_target) * 86400000)).toJSON().slice(0, 10))
        }
    }, [supplierData])
    return (
        <>
                ...
                <Grid item xs={12}>
                    <Box display='flex' width='100%' flexDirection='row' justifyContent='space-between'>
                        ...
                        <TextField
                            ...
                            className={classes.input}
                            value={taxPer}
                            onChange={(e) => setTaxPer(parseInt(e.target.value))}
                        />
                        <TextField
                            id="input-7"
                            label="Brutto Betrag"
