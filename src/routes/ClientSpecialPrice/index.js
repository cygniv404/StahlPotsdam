...
import {Divider, Grid, makeStyles, TextField, InputAdornment} from "@material-ui/core";
...
export default function (props) {
    ...
    const handleArticleChange = (e) => {
        ...
        if (e.target.value.length > 0) {
            get(`${props.sourceCollection}/${e.target.value.replace('_', '/')}`,
                (data) => {
                    if (data.id) {
    ...
    const loadArticle = (e) => {
        if (e.key === "ArrowDown") {
            get(`${props.sourceCollection}/${!articleId ? 0 : articleId.replace('/', '_')}/prev`,
                (data) => {
                    if (data.id) {
                        setArticleId(data.id);
                        setArticleData(data)
        ...
        if (e.key === "ArrowUp") {
            get(`${props.sourceCollection}/${!articleId ? 0 : articleId.replace('/', '_')}/next`,
                (data) => {
                    if (data.id) {
                        setArticleId(data.id);
                        setArticleData(data)
    ...
    const saveSpecialPrice = (e) => {
        if (e.key === "Enter" && e.target.value > 0) {
            post({
                ...
                [props.sourceCollection === 'article' ? 'amount' : 'amount_per']: parseFloat(specialPrice),
                unit: articleData.unit1 ?? articleData.unit,
            }, `client_special_price/${props.targetCollection}`, (data) => {
                if (data.id) {
    ...
    return (
        <>
            ...
            <Grid item xs={7}>
                <Grid item xs={12}>
                    <Box display='flex' flexDirection='row' justifyContent='space-between'>
                        {props.sourceCollection === 'article' && (
                            <TextField
                                id="label-7"
                                label="Artikelpreis"
                                type="number"
                                ...
                                InputProps={{
                                    endAdornment: <InputAdornment
                                        position="end">€/{articleData.unit1 ?? 'to'}</InputAdornment>,
                                }}
                                disabled
                                variant="outlined"
                        ...
                        <TextField
                            ...
                            InputProps={{
                                endAdornment: <InputAdornment
                                    position="start">{props.sourceCollection === 'article' ? `€/${articleData.unit1 ?? 'to'}` : '%'}</InputAdornment>,
                            }}
                            variant="outlined"
                            className={classes.input}
                            value={specialPrice}
