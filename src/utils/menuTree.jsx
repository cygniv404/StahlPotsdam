    ...
    Einkauf: {
      ...
      subMenus: {
        ...
        'Materialzugänge ändern': {
          ...
          navigationItems: [],
          component: <DataGridViewer
            showCreate={false}
            viewer="incoming_material"
            column={[
              {
        ...
        Kundenlisten: {
          ...
          categories: [
            ...
            },
            {
              ...
              component: <DataGridViewer
                showCreate={false}
                viewer="client_article_group_special_price"
                column={[
                  {
                    field: 'client_id',
                    headerName: 'Kundennummer',
    ...
    'Kunden Stammdaten': {
      ...
      subMenus: {
        ...
        Kundenwarengruppenrabatt: {
          ...
          navigationItems: [],
          component: <ClientSpecialPrice
            sourceCollection="article_groups"
...
function generateRoutes(menuNode, parentPath = '') {
  return Object.keys(menuNode).reduce((routes, key) => {
    const menuItem = menuNode[key];

    if (!Object.prototype.hasOwnProperty.call(menuItem, 'route')) {
      routes.push(...generateRoutes(menuItem, parentPath));
    }

    if (menuItem.route) {
      routes.push({
        path: parentPath + menuItem.route,
        navigationItems: menuItem.navigationItems,
        component: menuItem.component || <span id="input-1">{key}</span>,
      });
    }

    if (menuItem.subMenus) {
      const subRoutes = generateRoutes(menuItem.subMenus, parentPath);
      routes.push(...subRoutes);
    }

    if (menuItem.categories) {
      menuItem.categories.forEach((subCategory) => {
        routes.push({
          path: parentPath + subCategory.route,
...
export function getRoutes(pagePath = null) {
  const allRoutes = generateRoutes(menuTree);
  if (pagePath) {
    return allRoutes.filter((r) => r.path === `/${pagePath}`).pop();
  }
  return allRoutes;
}
