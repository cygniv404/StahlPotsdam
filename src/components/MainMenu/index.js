...
function DemoTabs(props) {
    const {labelId, onChange, selectionFollowsFocus, value, onKeyDown, autoFocus, tabRef, items} = props;
    const logout = () => {
        console.log('logout')
    }
    return (
        <AppBar position="static">
            <Tabs
                ...
                variant="fullWidth"
                onClick={() => {
                    return value === 4 ? logout() : null
                }}
                onKeyDown={onKeyDown}
            >
                {items.map((item, index) =>
                    (<Tab ref={value === index ? tabRef : null} autoFocus={index === 0 && autoFocus === 0} label={item}
                          aria-controls={`a11y-tabpanel-${index}`} id={`a11y-tab-${index}`}/>
                    ))}
            </Tabs>
        </AppBar>
    );
...
export default withRouter(AccessibleTabs)