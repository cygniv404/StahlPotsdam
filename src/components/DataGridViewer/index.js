...
import {DataGrid, deDE, GridToolbarContainer} from '@material-ui/data-grid';
import {Divider, Grid, makeStyles} from "@material-ui/core";
import Box from "@material-ui/core/Box";
import {get, post} from '../../utils/requests'
import {useSnackbar} from "notistack";
import {ExportToExcel, ShowCreateEntry, ShowDeleteEntry, ShowHideColumn, ShowUpdateAmount, ShowAmounts} from "./utils";
...
export default function BasicEditingGrid({
    ...
    const getAllRows = () => {
        return rows.map(({file_id, ...rest}) => rest);
    }
            ...
            const newRows = rows.map((item) => {
                if (item.id === id && item[field] !== value) {
                    if (field.indexOf('date') !== -1) {
                        value = new Date(new Date(value).setTime(new Date(value).getTime() + 1 * 86400000)).toJSON().slice(0, 10)
                    }
                    ...
                    }, (error) => enqueueSnackbar('Fehler: Feld kann nicht aktualisiert werden!', {variant: "error"}))
                }
            })
        },
        [rows],
            ...
            newColumns = [...newColumns,
                {
                    ...
                    headerName: 'Dokument',
                    width: 150,
                    renderCell: (params) => (<FileUploader viewer={viewer} setFileId={setFileId} fileId={params.value}
                                                           documentId={params.row.id} inDataGrid/>),
                },
            ]
        }
        setColumns(newColumns)
    ...
    useEffect(() => {
        if (viewer === 'orders_grid') {
            async function getMethodsLists() {
                const orderMethods = await get(`order_method/list`, (data) => {
                    return data.map((m) => m.method);
                }, null);

                const shippingMethods = await get(`shipping_method/list`, (data) => {
                    return data.map((m) => m.method)
                }, null);
