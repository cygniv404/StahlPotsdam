...
import {dlt, get, post, put} from "../../utils/requests";
...
export function ShowUpdateAmount({getSelectedRowData, handleTableUpdate, enqueueSnackbar, showButton}) {
        ...
        <>
            ...
            {updateOpen && (
                <Dialog open={updateOpen} onClose={handleUpdateClose} aria-labelledby="form-dialog-title">
                    <DialogTitle id="form-dialog-title">Rechnung #{invoiceData.id}</DialogTitle>
                    <DialogContent>
                        <TextField
                            autoFocus
                            margin="dense"
...
export function ShowCreateEntry({column, editableId, viewer, handleTableAdd, enqueueSnackbar}) {
    ...
    const handleCreate = () => {
        ...
        }, (error) => enqueueSnackbar('Fehler: Eintrag kann nicht erstellt werden!', {variant: "error"}))
    }
...
export function ShowDeleteEntry({selectionModel, viewer, handleTableDelete, enqueueSnackbar, disabled}) {
    const handleDelete = () => {
        if (selectionModel.length > 0) {
            ...
            }, (error) => enqueueSnackbar(`Fehler: Einträge können nicht gelöscht werden!`, {variant: "error"}))
...
export function ShowHideColumn({column, updateColumnVisibility, viewer, enqueueSnackbar}) {
                        ...
                        <FormGroup className={classes.formGroup}>
                            {
                                state.map((c) => {
                                    return (
                                        <FormControlLabel
                                            control={<Checkbox checked={c.checked}
                                                               onChange={handleChange}
                                                               name={c.field}
                                            />}
                                            label={c.headerName}
...
export function ShowAmounts({bruttoAmount, openAmount, deadline1Amount, deadline2Amount, deadline3Amount}) {
    const classes = useStyles()
    return (
        <>
            <TextField
